...
class BatchUtil:
    ...
    def _generate_set_object(self, workspace_name, obj_refs, set_name, set_type):
        ...
        # save KBaseSearch.GenomeSet instead of KBaseSets.GenomeSet
        save_search_set = False
        if set_type == 'genome':
            save_search_set = True
            elements = {}
            for obj_ref in obj_refs:
                elements[obj_ref] = {'ref': obj_ref}
            set_data['elements'] = elements
    ...
    def batch_import_assemblies_from_staging(self, params):
        ...
        if staging_subdir.startswith('/' + self.user_id + '/' ):
            temp = len(self.user_id) + 1
            staging_subdir = staging_subdir.replace('/' + self.user_id + '/' , '/') 
